   ...
   ResultSet rs;
    public CancelTickets(){
       ...
       con=Airline_Reservation_System.connectDb();
        ...
        JLabel top=new JLabel("Cancel Tickets");
        ...
        cancel.setBounds(200,450,100,20);
        add(cancel);
        cancel.setVisible(false);
        cancel.addActionListener(this);
        pnrlabel = new JLabel("Payment no");
        pnrlabel.setBounds(310,450,100,20);
        add(pnrlabel);
        pnrlabel.setVisible(false);
        pnr=new JTextField();
        pnr.setBounds(420,450,100,20);
        add(pnr);
        pnr.setVisible(false);
        confirm = new JButton("Confirm Cancellation");
        confirm.setBounds(340,500,170,30);
        add(confirm);
        confirm.setVisible(false);
        confirm.addActionListener(this);
    }
    int ecoseats=0,busseats=0;
    
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==confirm){
            if(pnr.getText().isEmpty()){
                JOptionPane.showMessageDialog(null,"Enter payment number");
            }else{
                String sql="select * from flight where flightno='"+fno.getText()+"'";
                try{
                    PreparedStatement pst=con.prepareStatement(sql);
                    rs=pst.executeQuery();
                    while(rs.next()){
                        ecoseats=rs.getInt("economyseats");
                        busseats=rs.getInt("businessseats");
                        ecoseats+=Integer.parseInt(eco.getText());
                        busseats+=Integer.parseInt(bus.getText());
                        String sql1="delete from ticket where ticketno='"+tkt.getText()+"' and paymentid='"+pnr.getText()+"'";
                        try{
                            
                            PreparedStatement pst1=con.prepareStatement(sql1);
                            pst1.executeUpdate();
                            String sql2="update flight set economyseats='"+ecoseats+"',businessseats='"+busseats+"' where flightno='"+fno.getText()+"'";
                            try{
